...
## 安装与启动（快速步骤）
1. 解压项目：
   - 将 KouriChat-1.4.3.2.zip 解压到目标目录（项目从 GitHub Release 页面下载：https://github.com/KouriChat/KouriChat/releases）。  
2. 进入目录并运行：
...
- 微信无法正常接入：检查微信客户端版本是否在 KouriChat 支持的范围内。KouriChat 通过自动化接口与微信桌面客户端通信，首次连接时需扫码登录。遇到连接失败，可尝试以下步骤：
    - 确保微信桌面客户端已登录且保持在前台运行
    - 在 KouriChat 后台执行 "微信掉线重连" 操作
    - 如仍不成功，查看控制台日志定位具体错误信息  
...
## 参考与资源
- KouriChat 官方仓库：https://github.com/KouriChat/KouriChat
- KouriChat Release 下载：https://github.com/KouriChat/KouriChat/releases  
